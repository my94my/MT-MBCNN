...
def extract_data(datafile, correct_nan_values=True):
    ...
    return raw_gdf.drop_channels(["EOG-left", "EOG-central", "EOG-right"])
...
def extract_events(label_file, gdf_events):
    ...
    trial_start_events = events[events[:, 2] == 2]  # 2 = 'Start of Trial, Trigger at t=0s'
    assert len(trial_start_events) == len(trial_events)
    artifact_trial_mask = zeros(len(trial_events), dtype=uint8)
    artifact_events = events[events[:, 2] == 1]  # 1 = 'Rejection of whole trial'
...
def load_gdf(data_folder, subject_id, start_offset_ms=-500, end_offset_ms=4000, low_cut_hz=0, high_cut_hz=60, noise_low=30, noise_high=40):
    ...
    noise2_data = []
    for et in ['T', 'E']:
        data_file = f'{data_folder}/A0{subject_id}{et}.gdf'
        label_file = f'{data_folder}/A0{subject_id}{et}.mat'
        ...
        marker = OrderedDict([("Left Hand", [1]), ("Right Hand", [2]), ("Foot", [3]), ("Tongue", [4])])
    ...
    return concatenate(data, axis=0), concatenate(label, axis=0), concatenate(noise2_data, axis=0), concatenate(noise1_data, axis=0)
